...
def multi_vectorstore_with_search() -> Generator[MultiVectorStore, None, None]:
    """MultiVectorStore fixture(scope="function") to test semantic search."""
    # Create a temporary directory for the test database
    temp_dir = tempfile.mkdtemp()
...
def test_add_documents_chunk(
    ...
    """Test adding documents to the vectorstore using chunking."""
...
def test_add_documents_summarize(
    ...
    """Test adding documents to the vectorstore using summarization."""
...
async def test_aadd_documents_summarize(
    ...
    """Test adding documents to the vectorstore usingg summarization."""
...
def test_add_documents_question(
    ...
    """Test adding documents to the vectorstore using questions."""
    ids = multi_vectorstore.add_documents(
...
def test_add_documents_multiple(
    ...
    """Test adding documents to the vectorstore using multiple functors."""
    func_list = [
        (
            "chunk",
            {
...
async def test_aadd_documents_multiple(
    ...
    ids = await multi_vectorstore.aadd_documents(docs, functor=func_list, llm=LLM)
...
def test_delete(multi_vectorstore: MultiVectorStore, docs: List[Document]) -> None:
    ...
    # Verify that the documents were deleted
...
def test_similarity_search(
    ...
    """Test for similarity search method."""
    ...
    # Verify that the number of results is correct
    assert len(results) <= 2
    assert len(results) > 0

    # Verify that the retrieved documents are the best match
    for doc in results:
...
def test_retriever(
    ...
    # Generate child documents for the parent document retrieved
    sub_docs = chunk(
        results[0],
        text_splitter="recursive",
        chunk_size=SMALL_CHUNK_SIZE,
